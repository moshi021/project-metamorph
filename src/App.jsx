...
// --- Main Component --------------------------------------------------------
export default function KidsGrowthLab() {
        ...
        <section className="md:col-span-3 space-y-4">
          <Card title="Recipe: Char vs Regolith">
            <div className="space-y-2">
              <Label>
                Char: <strong>{charPct}%</strong> · Regolith: <strong>{regolithPct}%</strong>
              </Label>
              <input
                type="range"
                min={0}
                max={100}
                value={charPct}
                onChange={(e) => setCharPct(clamp(parseInt(e.target.value || "0"), 0, 100))}
                className="w-full accent-emerald-600"
              />
              <Tip>
                Char = air spaces; Regolith = minerals. Try 70/30 for best results!
              </Tip>
            </div>
            <div className="grid grid-cols-3 gap-4 pt-3">
              <MiniStat label="Char" value={pct(charPct)} />
              <MiniStat label="Regolith" value={pct(regolithPct)} />
              <MiniStat label="Binder" value={pct(binderPct)} />
            </div>
          </Card>

          <Card title="Helper: Binder (Cotton Pulp)">
            <div className="space-y-2">
              <Label>
              ...
              <input
                ...
                max={30}
                value={binderPct}
                onChange={(e) => setBinderPct(clamp(parseInt(e.target.value || "0"), 0, 30))}
                className="w-full accent-amber-600"
              />
              <Tip>A little binder holds moisture—like a sponge! Try around 10%.</Tip>
            </div>
          </Card>
        ...
        <section className="md:col-span-2 space-y-4">
          ...
          <Card title="Why did that happen?">
            <ul className="space-y-2 text-sm">
              <Reason label="Char vs Regolith" value={Math.round(scores.char * 100)} expl="Roots need air spaces. 60–80% Char is friendliest." />
              <Reason label="Binder (sponge power)" value={Math.round(scores.binder * 100)} expl="A bit of pulp holds water and keeps crumbs together." />
...
function Label({ children }) { return <div className="text-sm text-slate-700 font-medium">{children}</div>; }
    ...
    <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
      ...
      {open && (
        <div className="mt-2 grid gap-2 text-sm text-emerald-900">
          <div>• <strong>Air spaces</strong> (from <em>Char</em>) let roots breathe.</div>
          <div>• <strong>Minerals</strong> (from <em>Regolith</em>) give building blocks.</div>
          <div>• <strong>Water</strong> is the drink; not too little, not too much.</div>
          <div>• <strong>Light</strong> is plant food from the sun (or lamps).</div>
  ...
  return (
    <header className="sticky top-0 z-10 backdrop-blur bg-gradient-to-r from-emerald-100 to-sky-100 border-b border-emerald-200">
      <div className="mx-auto max-w-5xl px-4 py-4 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-emerald-700">Kids Growth Lab</h1>
        <span className="ml-auto text-sm text-slate-700 italic">Learn by mixing & caring for your 🌱</span>
      </div>
    </header>
  );
}
  ...
  return (
    <footer className="mt-12 bg-gradient-to-r from-sky-50 to-emerald-50 border-t border-slate-200">
      <div className="mx-auto max-w-5xl px-4 py-8 text-center text-sm text-slate-600">
        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 inline-block">🌟 Try a challenge: Can you make your plant <strong>struggle</strong> on purpose, then fix it? Notice what changes!</div>
        <div className="mt-4 text-xs text-slate-500">Built with ❤️ using React & TailwindCSS</div>
      </div>
    </footer>
  );
}
